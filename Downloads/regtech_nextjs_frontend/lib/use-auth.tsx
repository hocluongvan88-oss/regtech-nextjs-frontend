...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      console.log("[v0] /api/auth/me response status:", response.status)

      if (response.ok) {
        const data = await response.json()
        console.log("[v0] User data received:", data.user.email)
        setUser(data.user)

        console.log("[v0] Setting auth context in localStorage")
        setAuthContext({
        ...
        console.error("[v0] Auth response not ok:", response.status, errorData)
      }
    } catch (error) {
